...
Seja $X_1,\ldots,X_n$ uma amostra de densidade $f(x;\theta)$, $\theta \in \Theta \subset \mathbb{R}$, onde:
\begin{enumerate}[label=(A\arabic*), leftmargin=0.9cm]
    \item $f(x;\theta)$ é três vezes diferenciável em $\theta$;
    \item derivadas podem ser permutadas com integrais: $\displaystyle \int \frac{\partial f(x;\theta)}{\partial \theta}dx = 0$ e $\displaystyle \int \frac{\partial^2 f(x;\theta)}{\partial \theta^2}dx = 0$;
    \item a informação de Fisher $I(\theta) = E_\theta\big[(\partial_\theta \log f(X;\theta))^2\big]$ é finita e positiva.
\end{enumerate}
Então existe uma solução $\hat{\theta}_n$ da equação de verossimilhança tal que
\[
\hat{\theta}_n \xrightarrow{P} \theta_0 \qquad \text{e} \qquad \sqrt{n}(\hat{\theta}_n - \theta_0) \xrightarrow{D} N\big(0, I^{-1}(\theta_0)\big).
\]
\end{teoremabox}
